...
// --- 1. CONFIG & TYPES ---
const STORAGE_VER = 'v10'; // 현재 버전
const PREVIOUS_VERSIONS = ['v9', 'v8', 'v7']; // 마이그레이션을 체크할 이전 버전들
...
interface Employee { id: string; name: string; email: string; password: string; role: Role; hireDate: string; }
...
const getEmployeeLeaveBalance = (hireDateStr: string, requests: LeaveRequest[], bonusRecords: BonusLeaveRecord[]): LeaveBalance => {
  ...
  return { accrued, bonus, usedAnnual, usedBonus, remainingAnnual: Number((accrued - usedAnnual).toFixed(1)), remainingBonus: Number((bonus - usedBonus).toFixed(1)) };
};
...
const getUnifiedHistory = (employee: Employee, requests: LeaveRequest[], bonusRecords: BonusLeaveRecord[]): UnifiedHistoryItem[] => {
  ...
  bonusRecords.forEach(br => { history.push({ id: br.id, date: format(parseISO(br.createdAt), 'yyyy-MM-dd'), type: 'EARNED', category: '보너스 연차', amount: br.amount, reason: br.reason }); });
  requests.forEach(req => {
...
const askGemini = async (prompt: string) => {
  ...
  } catch (error) { console.error('Gemini error:', error); return "AI 분석 정보를 가져올 수 없습니다."; }
};
...
export default function App() {
  ...
  const handleBackup = () => {
    const data = { employees, requests, bonusRecords, systemSettings };
  ...
  const handleSubmitRequest = async (formData: any, isEdit = false) => {
    if (!user) return;
    if (isEdit) {
      setRequests(p => p.map(req => req.id === formData.id ? { ...formData, status: LeaveStatus.PENDING, updatedAt: new Date().toISOString() } : req));
    } else {
      const newRequest: LeaveRequest = { ...formData, id: Math.random().toString(36).substr(2, 9), status: LeaveStatus.PENDING, employeeId: user.id, createdAt: new Date().toISOString() };
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <main className="flex-1 overflow-y-auto">
            <div className="max-w-5xl mx-auto px-4 py-6 md:py-8">
              <Routes>
                <Route path="/dashboard" element={<Dashboard employee={user} requests={requests} bonusRecords={bonusRecords.filter(b => b.employeeId === user.id)} onSubmitRequest={handleSubmitRequest} onDeleteRequest={(id: string) => setPendingDelete({id, type: 'REQUEST'})} />} />
                            ...
                            return (
                              <div key={emp.id} className="bg-white px-3 py-2.5 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-3 hover:border-blue-200 transition-all group">
                                ...
                                <div className="flex flex-wrap items-center gap-1.5 flex-1 justify-start md:justify-center">
                                  <AdminStat label="법정 발생" value={bal.accrued} color="text-slate-900" /><AdminStat label="보너스" value={bal.bonus} color="text-amber-600" /><AdminStat label="잔여 연차" value={bal.remainingAnnual} color="text-blue-600" /><AdminStat label="잔여 보너스" value={bal.remainingBonus} color="text-indigo-600" />
                                </div>
                                <div className="flex items-center gap-1 border-l md:pl-2 border-slate-100">
                                  ...
                                  <button onClick={(e) => { e.preventDefault(); e.stopPropagation(); setPendingDelete({id: emp.id, type: 'EMPLOYEE'}); }} className="p-2 text-slate-400 hover:text-red-500 transition-all" title="직원 삭제"><Trash2 size={16}/></button>
                                  <button onClick={() => setIsBonusModalOpen({empId: emp.id, name: emp.name})} className="ml-1 bg-slate-50 text-slate-500 px-2.5 py-1.5 rounded-md font-bold text-[10px] hover:bg-slate-900 hover:text-white transition-all border border-slate-100">보너스</button>
                                </div>
                              </div>
                            );
                          })}
      ...
      {isHistoryModalOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-[110] flex items-center justify-center p-4">
          <div className="bg-white w-full max-w-2xl rounded-2xl shadow-xl overflow-hidden max-h-[85vh] flex flex-col">
             <div className="px-6 py-4 border-b flex justify-between items-center bg-slate-50"><h3 className="font-bold text-sm">{isHistoryModalOpen.name}님의 상세 연차 기록</h3><button onClick={() => setIsHistoryModalOpen(null)}><X size={20}/></button></div>
             <div className="flex-1 overflow-y-auto p-4"><HistoryView employee={isHistoryModalOpen} requests={requests.filter(r => r.employeeId === isHistoryModalOpen.id)} bonusRecords={bonusRecords.filter(b => b.employeeId === isHistoryModalOpen.id)} isModal={true} /></div>
          </div>
        </div>
      )}
    </Router>
...
const Dashboard = ({ employee, requests, bonusRecords, onSubmitRequest, onDeleteRequest }: any) => {
  ...
  const balance = useMemo(() => getEmployeeLeaveBalance(employee.hireDate, myRequests.filter((r:any) => r.status === LeaveStatus.APPROVED), bonusRecords), [employee.hireDate, myRequests, bonusRecords]);
